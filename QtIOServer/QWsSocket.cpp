		...
		switch ( _version )
		{
			...
			case WS_V13:
			{
				...
				else
				{
                    ...
                    body.append( QIOServer::serializeInt( (int)closeStatusCode, 2 ) );
...
{
    //qDebug() << "QWsSocket:WRITE " << byteArray;
	if ( _version == WS_V0 )
	{
		QByteArray BA;
		BA.append( (char)0x00 );
...
void QWsSocket::processHandshake()
{
    ...
    //copy from QIOServer::dataReceived();
	...
	{
        qDebug() << "QWsSocket:processHandshake ERROR UPGRADE";
    ...
    QString accept = QIOServer::computeAcceptV4(key);
	if(accept != acceptFromServer)
	{
        qDebug() << "QWsSocket:processHandshake ERROR ACCEPT";
		emit error(QAbstractSocket::ConnectionRefusedError);
		return;
	}
    ...
    //qDebug() << "QWsSocket:processHandshake complete";
}
...
{
    //qDebug() << "QWsSocket:processDataV4 " << state();
	if( state() == QAbstractSocket::ConnectingState )
	{
		processHandshake();
	}
		...
		while (true)
        {
            //qDebug() << "QWsSocket:processDataV4 while";
			switch ( readingState ) {
				case HeaderPending: {
...
{
    qDebug() << "QWsSocket:processTcpStateChanged " << tcpSocketState;
